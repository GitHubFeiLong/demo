...
public class SumField {
	 ...
	 * @throws
	 */
	public static <T> Object getObject(List<T> list, String className) {
		try {
			Number [] nums = sum( list, className); 
	 ...
	 * @throws
	 */
	private static <T> Number[] sum(List<T> list, String clazzName) throws Exception {
		...
		for (T t : list) {
			...
			for (int i = 0; i < clazzFileds.length; i++) {
				...
				if (n != null) {
					// 相当于 nums[i] += n;
					nums[i] = SumField.getValue(n, nums[i], getType(n));
				}
